# Task 04 — Conectar Aba de Objetivos

**Objetivo:** Integrar Objectives Tab com a API, permitindo salvar e restaurar objetivos e pontos de dor operacional.
...
### Modificar
- `apps/web/src/features/clinic/components/settings/tabs/objectives-tab.tsx`

---

## Implementação

Substituir o componente `ObjectivesTab` para:

1. **Obter clinicId** da rota/context
2. **Carregar dados** com `useClinicObjectives()` ao montar
3. **Preencher estado local** com dados retornados
4. **Salvar dados** com `useSaveClinicObjectives()` ao clicar "Salvar"
5. **Mostrar loading/sucesso** durante a operação
...
1. **Import hooks:** `useClinicObjectives`, `useSaveClinicObjectives`
2. **Get clinicId:** Do route params via `useParams()`
3. **Load data:** `useEffect` atualiza estado local quando `savedData` muda
4. **Save handler:** Chama `saveObjectives(...)` com estado local
5. **Loading states:** Loader durante fetch e botão disabled durante save
6. **Fallback defaults:** Se não tem dados salvos, mostra defaults
...
- [ ] Ao montar, dados salvos aparecem no formulário (se existem)
- [ ] Carregar dados mostra spinner
- [ ] Drag-drop de prioridades funciona
- [ ] Selecionar pain points funciona
- [ ] Clicar "Salvar" chama API (pode verificar no Network tab)
- [ ] Após salvar, mensagem de sucesso aparece
- [ ] Ao voltar para a aba e reabrir, dados persistem
