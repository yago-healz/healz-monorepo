...
export class GetServicesTool extends StructuredTool {
  ...
  async _call(): Promise<string> {
    this.logger.debug(`[GetServicesTool] Fetching services for clinic ${this.clinicId}`)

    const data = await this.settingsService.getServices(this.clinicId)

    if (!data?.services) {
      this.logger.debug(`[GetServicesTool] No services found for clinic ${this.clinicId}`)
      return JSON.stringify({ services: [] })
    }

    const servicesList = Array.isArray(data.services) ? data.services : []
    this.logger.debug(`[GetServicesTool] Services retrieved for clinic ${this.clinicId}`, {
      servicesCount: servicesList.length,
      services: servicesList.map((s: any) => ({ name: s.name, duration: s.duration })),
    })
